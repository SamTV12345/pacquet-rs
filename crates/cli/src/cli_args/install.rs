...
impl InstallArgs {
    ...
    pub(crate) async fn run_with_preferred_versions(
        ...
        if should_parallelize_workspace_frozen_install(
            ...
            // Resolve each workspace importer sequentially. Each importer's
            // Install::run() extends the lockfile incrementally. Pruning is
            // skipped per-importer to avoid deleting sibling importers'
            ...
            for (importer_id, manifest_path) in &install_target_entries {
                ...
                if config.lockfile && !frozen_lockfile && !lockfile_only {
                    current_lockfile = Lockfile::load_from_dir(lockfile_dir)
